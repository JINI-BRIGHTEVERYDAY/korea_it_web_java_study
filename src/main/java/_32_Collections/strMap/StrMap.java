...
public class StrMap {
    ...
    // 키는 중복이 될 수 없다 Value는 중복이 가능하다, 순서도 없다
    public static void main(String[] args) {
        ...
        // 맵의 순회
        // 맵은 직접적으로 순회를 할 수 없다 => 직접 for-each를 돌릴 수가 없다
        // 왜냐하면, 순회하기 위해서는 Iterable<?> 인터페이스를 구현한 클래스이거나 배열타입 (T[])것들만 가능함
        // collection은 iterable인터페이스를 상속받음 (iterable은 반복이 가능하게 하는 인터페이스)
        // 근데 Map은 collection이 아님, 그래서 순회가 불가능함
        // 그럼에도 순회하고 싶으면?
        ...
        // entrySet()을 이용했을 때
        for (Map.Entry<String, String> : strMap1.entrySet()) {
            System.out.println("키 : " + entry.getKey() + " 값 : " + entry.getValue());
        }

        // entrySet 을 이용하게 되면 키값 전체 내용이 set형태로 오는데 keySet처럼  한 가지만 String으로
        // 들어가는것이 아님 그래서 키값형태를 받아줄 수 있는 Map.Entry에다가 넣어야 한다
        // Map.Entry는 Map에 들어가는 데이터의 형식인 키값 형태인것임
        ...
        //Map의 인덱스가 없다 => key로 접근하여 값을 출력해야 한다
        // => 순서도 없다

        // 특정 key의 존재여부
        boolean isContain1 = strMap1.containsKey("kor2025003");
        System.out.println("kor2025003의 존재 여부: " + isContain1);
